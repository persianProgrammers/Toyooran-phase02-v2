...
export function setAdminState(req: AdminRequest, key: string, value: unknown) { db.prepare('INSERT INTO admin_state(state_key,state_json,updated_at) VALUES(?,?,?) ON CONFLICT(state_key) DO UPDATE SET state_json=excluded.state_json, updated_at=excluded.updated_at').run(key, JSON.stringify(value), new Date().toISOString()); audit(req, 'state_update', key); if (key === 'products') { require('./catalog').triggerBackgroundCatalogGeneration(); } }
