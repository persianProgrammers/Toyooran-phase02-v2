...
import { catalogFilename, generateCatalogPdf, triggerBackgroundCatalogGeneration } from "./server/catalog";
import { z } from 'zod';

dotenv.config();

// Pre-generate the catalog PDF in the background
triggerBackgroundCatalogGeneration();
...
async function startServer() {
  ...
  // Dynamic catalog: rendered from the same product state used by the website/admin.
  app.get("/api/catalog.pdf", async (_req, res) => {
    try {
      const { pdf, productCount } = await generateCatalogPdf();
      res.set({
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${catalogFilename()}"`,
        "Content-Length": String(pdf.length),
        "Cache-Control": "no-store",
        "X-Catalog-Product-Count": String(productCount),
      });
      res.send(pdf);
    } catch (error) {
      console.error("Dynamic catalog generation failed", error);
      res.status(500).json({ error: "تولید کاتالوگ با آخرین اطلاعات محصولات ناموفق بود." });
    }
  });
